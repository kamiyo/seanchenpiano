...
goog.Gapi = function($rootScope, $q, $log, $window, gapiUrl) {
...
cal.service('parseEventTime', function() {
...
cal.service('getCal', ['$log', '$rootScope', 'Gapi', '$q', 'calParams', 'parseEventTime', function($log, $rootScope, Gapi, $q, calParams, parseEventTime) {
	var calList;
...
mp.constant('smJs', './scripts/soundmanager2-nodebug-jsmin.js');

mp.Player = function($rootScope, $q, $log, $filter, swfUrl, smJs, $timeout) {
	this.player;
...
.controller('navigation', ['$scope', '$log', 'menuEntries', function($scope, $log, menuEntries) {
	...
	this.show = function() {
		angular.element('#front').removeClass('animate-show').addClass('animate-hide').bind("webkitTransitionEnd oTransitionEnd otransitionend transitionend msTransitionEnd", function(event){
	...
	$rootScope.$on('$routeChangeSuccess', function(event, next, current) {
		...
		scrollFn.restore();
		if (!start) {
			if (next.loadedTemplateUrl == "/about.php") {
				$('.quote').removeAttr('style').css({top: '20px', left: '50px'});
				$scope.front = fronts[3];
			} else if (next.loadedTemplateUrl == "/schedule.php") {
				$('.quote').removeAttr('style').css({bottom: '20px', left: '50px'});
				$scope.front = fronts[1];
			} else if (next.loadedTemplateUrl == "/press.php") {
				$scope.front = fronts[2];
				$('.quote').removeAttr('style').css({display: 'none'});
			} else if (next.loadedTemplateUrl == "/home.php") {
				$scope.front = fronts[0];
				$('.quote').removeAttr('style').css({top: '20px', left: '50px'});
			}
		...
		if (next.loadedTemplateUrl != "/videos.php") {
			if (!prevVid) {
				return;
			}
			prevVid = false;
...
.controller('calendar', ['$scope', '$location', '$timeout', 'days', 'cService', 'getCal', 'scrollFn', function($scope, $location, $timeout, days, cService, getCal, scrollFn) {
	...
	getCal.get().then(function(result) {
		...
		for (var i = 0; i < 3; i++) {
		    ...
		    $scope.listCalendar[i].location = $scope.listCalendar[i].location.split(",")[0];
		}
