					...
					$rootScope.fadePromise = $.Velocity.animate(
						...
						{ opacity: 0.0 },
						{
							...
							complete: function (elements) {
								$rootScope.$apply($location.search(''));
...
root.config(['$locationProvider', '$routeProvider', function ($locationProvider, $routeProvider) {
	...
	$routeProvider.when("/about", {
		...
		resolve: {
			list: function ($q, $log, $http, musicResource, discResource) {
			    ...
			    $http.get('bio.php?q=long').success(function (data) {
				    bioD.resolve(data);
				})
				.error(function () {
				    bioD.reject('could not find bio.php');
				});

				return $q.all([bioD.promise, concertoD.promise, soloD.promise, chamberD.promise, composingD.promise, arrangingD.promise, videogameD.promise, discD.promise]);
			}
		},
		...
		controller: "homeCtrl",
		resolve: {
			bio: function ($q, $http, bioResource) {
				var deferred = $q.defer();
				//var cb = function (result) {
				//    console.log(result);
				//    deferred.resolve(result);
				...
				$http.get('bio.php').success(function (data) {
				    data = data.concat('<p align="right"><a href="/about?q=bio">read the full bio...</a></p>');
					deferred.resolve(data);
				})
				.error(function () {
					deferred.reject('could not find bio.php');
				});
...
root.controller('homeCtrl', ['$scope', 'bio', function ($scope, bio) {
	$scope.bio = bio;
...
//	}
	...
	.controller('aboutCtrl', ['$scope', 'list', '$location', '$timeout', function ($scope, list, $location, $timeout) {
		...
		$scope.$on('$routeChangeSuccess', function () {
			$timeout(function () {
				if ($location.search().q == "bio") {
					$('#biocontainer').trigger('click');
				}
			}, 0);
		});
...
}).factory('bioResource', function ($resource) {
	return $resource('bio.php', {}, {
		query: { method: 'GET', params: {}, isArray: true }
	});
